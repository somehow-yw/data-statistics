    ...
    <Tabs :animated="false">
        <Tab-pane label="咨询统计">
			...
			<div class="selectlist mainselect">
			    ...
			    <Date-picker  format="yyyy-MM-dd" @on-change="startdayChange" v-model="params1startday" type="date" placeholder="选择日期" style="width: 150px"></Date-picker>至
			    <Date-picker format="yyyy-MM-dd" @on-change="enddayChange" v-model="params1endday" type="date"  placeholder="选择日期" style="width: 150px;margin-left:20px"></Date-picker>
			    ...
			    <Select  @on-change="cityChange" style="width:150px" clearable placeholder="所有市">
			        <Option v-for="item in cityList" :value="item.buyer_city" :key="index">{{ item.buyer_city }}</Option>
			    </Select>
			    <Select  @on-change="districtChange" style="width:150px" clearable placeholder="所有区">
			        <Option v-for="item in districtList" :value="item.buyer_district" :key="index">{{ item.buyer_district }}</Option>
			    </Select>
        ...
        <Tab-pane label="咨询排行">
        	...
        	<div class="selectlist mainselect">
        		<Select v-model="dayslenthmodel" style="width:150px" @on-change="dayslenthChange2">
    			  ...
    			  <Date-picker  format="yyyy-MM-dd" @on-change="startdayChange2" v-model="params2startday" type="date" placeholder="选择日期" style="width: 150px"></Date-picker>至
			    <Date-picker format="yyyy-MM-dd" @on-change="enddayChange2" v-model="params2endday" type="date"  placeholder="选择日期" style="width: 150px;margin-left:20px"></Date-picker>
			    <Select  @on-change="proviceChange2" clearable style="width:150px" placeholder="所有省/市">
			     ...
			     <Select  @on-change="cityChange2" clearable style="width:150px" placeholder="所有市">
			        <Option v-for="item in cityList2" :value="item.buyer_city" :key="index">{{ item.buyer_city }}</Option>
			    </Select>
			    <Select  @on-change="districtChange2" clearable style="width:150px" placeholder="所有区">
			        <Option v-for="item in districtList2" :value="item.buyer_district" :key="index">{{ item.buyer_district }}</Option>
			    </Select>
                <Button type="success" @click="baseRankselect">确认筛选</Button>
        	</div>
        	<div class="selectlist"><div><h4>基础咨询</h4></div>
            </div>
            <Tabs type="card" :animated="false" @on-click="tabclick">
                <Tab-pane label="按咨询商品">
                     ...
                     <Page :current="params2.page" :total="basetotal" :page-size="10" class-name="page" simple show-elevator @on-change="basechangePage"></Page>
                </Tab-pane>
                <Tab-pane label="按咨询品牌">
                    <Table :columns="brandcolumns" stripe size="small" :data="goodsdata" :context="self">
                    </Table>
                    <Page :current="params2.page" :total="basetotal" class-name="page" :page-size="10" simple show-elevator @on-change="basechangePage"></Page>
                </Tab-pane>
    		...
    		<Button type="text">咨询买家</Button>
		        ...
		        <Option v-for="item in buyerConsultList" :value="item.value" :key="item">{{ item.label }}</Option>
		    </Select>
		    </div>
		    <Card :bordered="true" style="width:100%">
    			<Spin size="large" fix v-if="spinShow3"></Spin>
    			<div style="width: 100%">
    				 <Table stripe :context="self"  :columns="buyerConsultcolumns" :data="buyerConsultdata"></Table>
    				 <Page :current="buyerparams.page" :total="buyertotal" :page-size="10" class-name="page"  show-elevator @on-change="buyerchangePage" simple></Page>
    			</div>
    		</Card>
    		</div>
    		...
    		<div>
    			...
    			<div class="selectlist mainselect">
    			  ...
    			  <Date-picker  format="yyyy-MM-dd" @on-change="startdayChange3" v-model="params3startday" type="date" placeholder="选择日期" style="width: 150px"></Date-picker>至
			    <Date-picker format="yyyy-MM-dd" @on-change="enddayChange3" v-model="params3endday" type="date"  placeholder="选择日期" style="width: 150px;margin-left:20px"></Date-picker>
                ...
                <div @click="provinceSelect">
			     <Select  @on-change="marketChange3" style="width:150px"  clearable placeholder="全部市场">
			        <Option v-for="item in marketList" :value="item.seller_market" :key="index">{{ item.seller_market }}</Option>
			    </Select>
                </div>
    			...
    			<div style="width: 100%;">
    				 <Table stripe :context="self"  :columns="sellerConsultcolumns" :data="sellerConsultdata"></Table>
    				 <Page :current="params3.page" :total="sellertotal" class-name="page" :page-size="10" show-elevator simple  @on-change="sellerpageChange"></Page>
    			</div>
    		</Card>
    		</div>
        </Tab-pane>
    ...
    export default {
        data () {
        	return {
        		...
        		buyerConsultList: [
                   {value: '一批',label:'一批'},{value:'二批',label:'二批'},{value:'全部',label:'全部'}
                    ], //买家咨询的下拉选择
        		total:0,
                buyertotal: 0,
                sellertotal: 0,
        ...
        methods: {
            ...
            buyerChange (e) {
                if(e=='全部') {delete this.buyerparams.filter.buyer_type;}
                else {this.buyerparams.filter.buyer_type = [e];}
