     ...
     <div class="layout-header">
        ...
        <Tab-pane label="销售排行">
            ...
            <Card :bordered="true">
                ...
                <Date-picker  format="yyyy-MM-dd" @on-change="startdayChange1" v-model="startday1" type="date" placeholder="选择日期" style="width: 150px"></Date-picker>至
                <Date-picker format="yyyy-MM-dd" @on-change="enddayChange1" v-model="endday1" type="date"  placeholder="选择日期" style="width: 150px;margin-left:20px"></Date-picker>
                ...
                <div><h6>销售数据</h6>
                    <Tabs :animated="false" type="card" @on-click="tabchange">
                        <Tab-pane label="销售商品">
                            <Table :columns="sellgoodscolumns"  :data="selldata" :context="self" @on-sort-change="sortchange"></Table>
                            <Page :current="baserankparams.page" class-name="page"  show-elevator :page-size="10" :total="selltotal" @on-change="basePagechange" simple></Page>
                            <Spin size="large" fix v-if="basespinShow"></Spin>
                        </Tab-pane>
                        <Tab-pane label="销售品牌">
                             <Table :columns="sellbrandcolumns" :data="selldata" :context="self" @on-sort-change="sortchange"></Table>
                             <Page :current="baserankparams.page" :page-size="10" :total="selltotal" class-name="page"  @on-change="basePagechange" show-elevator simple></Page>
                             <Spin size="large" fix v-if="basespinShow"></Spin>
                        </Tab-pane>
                        <Tab-pane label="销售分类">
                            <Table :columns="sellsortcolumns" :data="selldata" :context="self" @on-sort-change="sortchange"></Table>
                            <Page :current="baserankparams.page" :page-size="10" :total="selltotal" class-name="page" @on-change="basePagechange" show-elevator simple></Page>
                            <Spin size="large" fix v-if="basespinShow"></Spin>
                        </Tab-pane>
                    </Tabs>
                    ...
                    <div class="selectlist">
                        <div><h5>买家排行</h5></div>
                            ...
                            <Select  @on-change="payWayChange" clearable style="width:150px" placeholder="全部支付方式">
                                <Option v-for="item in paywayList" :value="item.value" :key="index">{{ item.label }}</Option>
                            </Select>
                    </div>  <!-- selectlist end-->
                    
                    <div>
                        <Table border stripe :columns="buyerdatacolumns" :data="buyerdata" :context="self"></Table>
                        <Page :current="buyerrankparams.page" :total="buyertotal" class-name="page" :page-size="10" show-elevator simple  @on-change="buyerpageChange"></Page>
            ...
            <Card :bordered="true">
                ...
                <Date-picker  format="yyyy-MM-dd" @on-change="startdayChange2" v-model="startday2" type="date" placeholder="选择日期" style="width: 150px"></Date-picker>至
                <Date-picker format="yyyy-MM-dd" @on-change="enddayChange2" v-model="endday2" type="date"  placeholder="选择日期" style="width: 150px;margin-left:20px"></Date-picker>
                ...
                <Select  @on-change="marketChange2" style="width:150px" placeholder="所有市场">
                    <Option v-for="item in marketList" :value="item.market" :key="index">{{ item.market }}</Option>
                </Select>
                ...
                <!-- selectlist end -->
                <div>
                     <Table border stripe :columns="sellererdatacolumns" :data="sellerdata" :context="self" @on-sort-change="sellersortchange"></Table>
                     <Page :current="sellerrankparams.page" :total="sellertotal" class-name="page" :page-size="10" show-elevator simple  @on-change="sellerpageChange"></Page>
            ...
            <Card :bordered="true">
                ...
                <Date-picker  format="yyyy-MM-dd" @on-change="startdayChange3" v-model="startday3" type="date" placeholder="选择日期" style="width: 150px"></Date-picker>至
                <Date-picker format="yyyy-MM-dd" @on-change="enddayChange3" v-model="endday3" type="date"  placeholder="选择日期" style="width: 150px;margin-left:20px"></Date-picker>
                <Select  @on-change="paywayChange3" clearable style="width:150px" placeholder="全部支付方式">
                 ...
                 <div>
                    ...
                    <div>
                        <Table border stripe :columns="provinceRankdatacolumns" :data="provincedata" :context="self"  @on-sort-change="provincesortchange"></Table>
                 ...
                 <div>
                    <div class="selectlist">
                     ...
                     <div>
                         <Table border stripe :columns="cityRankdatacolumns" :data="cityRankdata" :context="self" @on-sort-change="citysortchange"></Table>
                 ...
                 <div>
                    ...
                    <div>
                         <Table border stripe :columns="districtRankdatacolumns" :data="districtRankdata" :context="self" @on-sort-change="districtsortchange"></Table>
    ...
    export default {
        data () {
            return {
                ...
                sell: 'goods',
                 ...
                 cityList: [],
                 cityList2 : [],
                 typeList: [{value:11, label:"一批"},{value:12, label:"厂家"},{value:21,label:"二批"},{value:22,label:"第三方"},{value:23,label:"配送公司"},{value:24,label:"终端"},{value:25,label:"餐厅"},{value:26,label:"商超零售"},{value:31,label:"司机"},{value:100,label:"直营"},{value:0,label:'全部店铺'}],
                 marketList: [],
                 paywayList: [{value:1,label:"付款到平台"}, {value:2,label:"上车收钱"},{value:3,label:"自行协商"},{value:100,label:'全部支付方式'}],
                 districtList: [],
                 ...
                 sellbranddata: [],
                 sellsortdata: [],
                 sellgoodscolumns: [{title: '排名', width:160, render (row, column, index) {
                            return `<div>{{(baserankparams.page-1)*10+${index}+1}}</div>`;
                        }}, {title: '商品', width:200, key: 'goods_name'}, {title: '销售额(元)', width:160, key: 'amount',sortable:'custom'}, {title: '销售数量', sortable:'custom', width:160, key: 'num'}, {title: ' ', render (row, column, index) {
                            return `<div><p style="background: gray;height:30px;width:${row.realrate}%"></p></div>`}}],
                sellbrandcolumns:  [{title: '排名', width:160, render (row, column, index) {
                            return `<div>{{(baserankparams.page-1)*10+${index}+1}}</div>`;
                        }}, {title: '品牌',  width:200, key: 'brand'}, {title: '销售额(元)', width:160, key: 'amount',sortable:'custom'}, {title: '销售数量', sortable:'custom', width:160, key: 'num'}, {title: ' ', render (row, column, index) {
                            return `<div><p style="background: gray;height:30px;width:${row.realrate}%"></p></div>`}}],
                sellsortcolumns: [{title: '排名', width:160, render (row, column, index) {
                            return `<div>{{(baserankparams.page-1)*10+${index}+1}}</div>`;
                        }}, {title: '分类', width:200, key: 'sort'}, {title: '销售额(元)', width:160, key: 'amount',sortable:'custom'}, {title: '销售数量', sortable:'custom', width:160, key: 'num'}, {title: ' ', render (row, column, index) {
                            return `<div><p style="background: gray;height:30px;width:${row.realrate}%"></p></div>`}}],           
                buyerdatacolumns:[{title: '排名',render (row, column, index) {
                            return `<div>{{(buyerrankparams.page-1)*10+${index}+1}}</div>`;
                        }}, {title: '店铺名', key: 'dianPuName'}, {title: '联系电话', key: 'user_tel'},{title: '购买金额', key: 'amount'},{title: '购买数量', key: 'num'}, {title: '订单笔数', key: 'order'}],
                sellererdatacolumns: [{title: '排名',render (row, column, index) {
                            return `<div>{{(sellerrankparams.page-1)*10+${index}+1}}</div>`;
                        }}, {title: '店铺名', key: 'dianPuName'}, {title: '联系电话', key: 'user_tel'},{title: '销售金额', key: 'amount',sortable:'custom'},{title: '销售数量', sortable:'custom',  key: 'num'}, {title: '订单笔数', key: 'order'}],
                provinceRankdatacolumns: [{title: '排名',type: 'index'}, {title: '省市', key: 'province'}, {title: '销售金额', key: 'amount',sortable:'custom'},{title: '销售数量', sortable:'custom', key: 'num'}, {title: '订单笔数', key: 'order'}],
                cityRankdatacolumns: [{title: '排名',type: 'index'}, {title: '省市', key: 'city'}, {title: '销售金额', key: 'amount',sortable:'custom'},{title: '销售数量', sortable:'custom', key: 'num'}, {title: '订单笔数', key: 'order'}],
                districtRankdatacolumns: [{title: '排名',type: 'index'}, {title: '区县', key: 'county'}, {title: '销售金额', key: 'amount',sortable:'custom'},{title: '销售数量', sortable:'custom', key: 'num'}, {title: '订单笔数', key: 'order'}],
                selldata: [],
                cityRankdata: [],
                baserate: 0,
            }
        ...
        methods: {
            shopTypeChange (e) { //买家排行--店铺类型改变
                if (e == ""|| e=='0') {delete this.buyerrankparams.filter.trenchnum;return;}
            ...
            payWayChange (e) {//买家排行--支付方式改变
                if (e == "" || e == '100') {delete this.buyerrankparams.filter.method;this.getBuyerData();return;}
                var obj = this.buyerrankparams.filter;
            ...
            paywayChange2 (e) {  //卖家排行-支付方式改变
                if (e == "" || e == '100') {delete this.sellerrankparams.filter.method;return;}
                var obj = this.sellerrankparams.filter;
                 obj = Object.assign(obj, {'method':[e]});
                 this.sellerrankparams.filter = obj;
            },
              paywayChange3 (e) {
                if (e =="" || e == '100'){delete this.districtrankparams.filter.method;
                    delete this.provincerankparams.filter.method;
                    delete this.cityrankparams.filter.method; return;}
            ...
            city_provinceChange (e) {
                if (e == '全部') {delete this.cityrankparams.filter.province;return;};
            ...
            district_provinceChange (e) {
                this.cityList2 = [];
                if (e == '全部') {delete this.districtrankparams.filter.province;return;};
            ...
            district_cityChange (e) {
                if (e == '全部') {delete this.districtrankparams.filter.city;return;};
                var obj = this.districtrankparams.filter;
            ...
            proviceChange1 (e) {   //基础排行-省份改变
                this.cityList = [];
                if(e == '全部') {delete this.baserankparams.filter.buyer_province; return;}
            ...
            proviceChange2 (e) {  //卖家排行-省份改变
                this.marketList = [];
                 if(e == '全部') {delete this.sellerrankparams.filter.province; return;}
                 var obj = this.sellerrankparams.filter;
                 obj = Object.assign(obj, {'province':[e]});
                 this.sellerrankparams.filter = obj;
            ...
            cityChange1 (e) {  //基础排行-市改变
                this.districtList = [];
                if(e == '全部') {delete this.baserankparams.filter.buyer_city; return;}
                var obj = this.baserankparams.filter;
            ...
            marketChange2 (e) {  //卖家排行-市场改变
                if(e == '全部') {delete this.sellerrankparams.filter.pianquId; return;}
                 var obj = this.sellerrankparams.filter;
                 obj = Object.assign(obj, {'pianquId':[e]});
                 this.sellerrankparams.filter = obj;
            },
            districtChange1 (e) {  //基础排行-区县改变
                if(e == '全部') {delete this.baserankparams.filter.buyer_district; return;}
                var obj = this.baserankparams.filter;
                    ...
                    if(response.body.code == 0) {
                        ...
                        if (response.body.data.current_page == 1 && _this.baserankparams.order == 'num') _this.baserate = response.body.data.detail[0].num;
                            if(_this.baserankparams.order == 'amount') {
                                    for(var i=0,len=_this.selldata.length;i<len;i++){
                                var obj = _this.selldata[i];
                                var realrate = _this.selldata[i].amount/_this.baserate*100;
                            ...
                            if (_this.baserankparams.order == 'num') {
                                    for(var i=0,len=_this.selldata.length;i<len;i++){
                                var obj = _this.selldata[i];
                                var realrate = _this.selldata[i].num/_this.baserate*100;
                                obj = Object.assign(obj,{'realrate':realrate});
                                _this.selldata[i] = obj;
                               }
                               }
