    ...
    export default{
        data () {
            return {
                ...
                provincecolumns:[{ title: '省市', width:150,  key: 'province'}, { title: '店铺数', key: 'num'}, { title: '   ', render (row, column, index) {
                ...
                { title: '占比', render (row, column, index) {
                           return `<div>${row.zhangbi}%</div>`;
                        }},
                { title: '   ', render (row, column, index) {
                           return `<div><p style="background: gray;height:20px;width:${row.realrate}%"></p></div>`;
                        }}],
        ...
        methods: {
                ...
                _this.$http({
                    ...
                    if (response.body.code == 0) {
                       _this.$Loading.finish();
                        ...
                        for(var i=0,len=_this.citydata.length;i<len;i++) { _this.cityShopTotal +=_this.citydata[i].num;}
                       for(var i=0,len=_this.citydata.length;i<len;i++){
                ...
                _this.$http({
                    ...
                    if (response.body.code == 0) {
                       _this.$Loading.finish();
                        ...
                        for(var i=0,len=_this.districtdata.length;i<len;i++){ _this.districtShopTotal += _this.districtdata[i].num;}
                       for(var i=0,len=_this.districtdata.length;i<len;i++){
            ...
            initLinesCharts () {
                ...
                var totalLinesData = _this.linesdata,
                    ...
                    yNewTwo = [],
                    yNewRes = [];
                   for(var i =  0,len=totalLinesData[11].length;i<len;i++){
                    xTime.push(totalLinesData[11][i].time);
                    yNewOne.push(totalLinesData[11][i].num)
                   }
                   for(var i =  0,len=totalLinesData[21].length;i<len;i++){
                    yNewTwo.push(totalLinesData[21][i].num)
                   }
                   for(var i =  0,len=totalLinesData[25].length;i<len;i++){
